...
public final class DefaultSpellActions {
            ...
            if (delay > 0) {
                new Thread(() -> {
                    for (int i = 0; i < amount; i++) {
                        ...
                        if (entity instanceof SpectralArrowEntity arrow) {
                            arrow.pickupType = PersistentProjectileEntity.PickupPermission.CREATIVE_ONLY;
                        }
    ...
    @SpellAction(id="teledash")
    public static void teleDash(Spell.SpellParameterProvider parameterProvider, Float dist) {
        PlayerEntity player = parameterProvider.player();
        World world = player.getWorld();
        if (!world.isClient()) {
            world.playSound(null, player.getBlockPos(), SoundEvents.ENTITY_ENDERMAN_TELEPORT, SoundCategory.PLAYERS, 1f, 1f);
            Vec3d newPos = player.getPos().add(player.getRotationVector().multiply(dist));
